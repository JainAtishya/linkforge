...
const { REFRESH_TOKEN_COOKIE, clearAuthCookies } = require("../utils/cookies");

const { generateGoogleAuthUrl } = require("../services/google.service");
...
const logoutAllUsers = asyncHandler(async (req, res) => {
  ...
  return res
    .status(StatusCodes.OK)
    .json(new ApiResponse(StatusCodes.OK, null, "Logged out from all devices"));
});
...
const googleCallback = asyncHandler(async (req, res) => {
  ...
  if (!state || !storedState || state !== storedState) {
    ...
    throw new ApiError(StatusCodes.UNAUTHORIZED, "Invalid OAuth state");
  }

  if (!code) {
    clearOAuthStateCookie(res);

    throw new ApiError(StatusCodes.BAD_REQUEST, "Authorization code missing");
  }
...
const getCurrentUser = asyncHandler(async (req, res) => {
  const user = await authenticateUser(req.user.sub);
